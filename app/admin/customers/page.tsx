  ...
  const { data } = useQuery({
    ...
    queryFn: () =>
      api.getPaged<{ id: string; firstName: string; lastName: string; nationalCode: string; mobile: string; createdAt: string }[]>(
        `/api/v1/admin/customers?page=1&pageSize=20&search=${encodeURIComponent(search)}`
      ),
  });
  ...
  return (
    <AdminShell>
      <div className="flex flex-col gap-4">
        ...
        <SearchField value={search} onChange={setSearch} placeholder="جستجو نام، کد ملی، موبایل..." />
        {items.length === 0 ? (
          <Empty>
            <EmptyHeader>
